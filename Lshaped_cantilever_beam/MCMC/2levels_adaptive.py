...
lik_min = -49.361669728681775
delta_max = 53.58127621619366
...
class custom_loglike_coarse:
    ...
    def loglike(self,Input_HF):
        ...
        eps = 1e-5
        loglike_value_coarse_adapted = loglike_value_coarse - eps*((theta_coarse-mean)**2)/(std**2)
        # print(f"loglik_adapted: {loglike_value_coarse_adapted}")
...
# Adaptive Metropolis -> più lento, ma ESS più alta, modificare file utils di tinyda per farlo funzionare
# am_cov = np.eye(1)
# am_t0 = 100
# am_sd = None
# am_epsilon = 1e-6
...
my_chains = tda.sample(my_posteriors, my_proposal, iterations=iterations, n_chains=5,store_coarse_chain=True,force_sequential=True)
