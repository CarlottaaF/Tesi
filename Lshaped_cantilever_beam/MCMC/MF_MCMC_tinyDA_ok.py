# -*- coding: utf-8 -*-
"""
Created on Wed Nov  6 17:49:16 2024
...
#MCMC parameters
n_chains = 1 #number of starting locations to compute the EPSR
...
def single_param_like_single_obs(obs,mapping): #likelihood assumes independent prediction errors 
    for j1 in range(n_chains):
        for i1 in range(n_channels):
            ...
            like_single_dof[j1,i1] = 1./(np.sqrt(2.*np.pi)*rmse) * np.exp(-((rsse**2)/(2.*(rmse**2))))*1e35
...
my_model = MySurrogateModel(n_channels, signal_resolution, N_entries,LF_signals_start,HF_net_to_pred)
my_loglike = custom_loglike(signal_resolution, n_channels, N_obs, N_entries, limit, Y_HF,LF_signals_start)
...
for i in range(len(params[0, :])):
    log1[i] = my_loglike.loglike(my_model.__call__(params[0,i]))
lik1 = np.exp(log1)
...
for i in range(len(params[1, :])):
    log2[i] = my_loglike.loglike(my_model.__call__(params[1,i]))
lik2 = np.exp(log2)

plt.figure(figsize=(8, 5))
plt.plot(params[1,:], lik2, marker='o', linestyle='', label="Likelihood")
plt.xlabel("Parametro del modello")
plt.ylabel("Likelihood")
plt.title("Andamento della Likelihood(catena2) al variare del parametro")
plt.legend()
plt.show()  