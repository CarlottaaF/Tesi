...
def setup_environment():
    ...
    np.random.seed(123)
    n_samples = 128000
    resolutions = [(50, 50), (30, 30), (20,20), (15, 15)]
    field_mean, field_stdev, lamb_cov = 1, 1, 0.1
    mkl_values = [16, 16, 16, 16]
...
def generate_samples(n_samples):
    return norm_dist(loc=0, scale=1).ppf(lhcs(16, samples=n_samples)) #cambiare qua il numero!!
...
def generate_solver_data(solvers, solver_key, samples, datapoints, path_prefix):
    ...
    np.savetxt(f"{path_prefix}/X_train_{solver_key}_16_3.csv", samples[:split_idx], delimiter=",")
    np.savetxt(f"{path_prefix}/y_train_{solver_key}_16_3.csv", data[:split_idx], delimiter=",")
    np.savetxt(f"{path_prefix}/X_test_{solver_key}_16_3.csv", samples[split_idx:], delimiter=",")
    np.savetxt(f"{path_prefix}/y_test_{solver_key}_16_3.csv", data[split_idx:], delimiter=",")
...
def main():
    ...
    for key in ["h1"]:
        generate_solver_data(solvers, key, samples, datapoints, "/data_generation_cartella")
